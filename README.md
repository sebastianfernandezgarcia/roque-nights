...
| 3 | `rank_nights` | You need the best night in a date range, scored by dark hours free of Moon. Honours `AbortSignal`. | readOnly, idempotent | always |
...
The observing site is changed through WebMCP's **declarative** API rather than a 15th tool. The `<form>` in the site dialog carries `toolname="set_observing_site"`, stays mounted whether the dialog is open or not, and answers the `agentInvoked` event with `respondWith`. The agent fills in the same fields the human does, so there is one validation path, one set of error messages, and one source of truth. Coordinates without an IANA time zone are accepted, and every local time from then on is `null` with a caveat that says so, because inventing a time zone is worse than admitting you do not know one.
...
src/state/store.ts   one vanilla Zustand store: site, night, time, view, plan,
       ...
       +-------------------------------+
       |                               |
src/ui/*, src/sky/*              src/tools/*  (14 tools)
...
- The 14 tools, the declarative form, the contextual registration, export, import and the share link are verified end to end in a real Chromium with the WebMCP testing flag, by the audit script above.
